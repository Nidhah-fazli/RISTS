...
public class Generator {
    ...
    private final double compressRatio = 0.2;

    private final int blockDistance = 3;
...
//        
//        String wn = new String("C:\\Program Files (x86)\\WordNet\\2.1\\bin\\wn " + term + " -synsn");
...
public ArrayList<String> setKeywords(String[] keywords){
  ...
  //  String[] synonyms = generateSynonyms(keywords);
    ...
    for(int i = 0; i < keywords.length; i++){
        ...
        keywords[i] = keywords[i].replace("[","").replace("]", "").replace("(", "").replace(")", "").replace("{", "").replace("}", "").replace(".", " ").replace(",", " ").replace(";", "").replace("!", "").replace("?", "").replace("<", "").replace(">", "").replace("%", "").replace("-", "");
        resultTerm = tp.preprocess(keywords[i]);
 
        //System.out.println(keywords[i]+":- after stemming & stop word removal :"+resultTerm);
        if((resultTerm!=null) && (keywords[i].length()>2))   
            al.add(resultTerm);
